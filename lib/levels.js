...
const LAYOUTS = [
  { par: 7, art: ["VHHV", "VHHV", "VSSV", "V..V", "...."] },
  { par: 17, art: ["VHHV", "VHHV", "VSSV", "VSSV", "...."] },
  { par: 26, art: ["VHHV", "VHHV", "VGGV", "VS.V", "..S."] },
  { par: 51, art: ["VHHV", "VHHV", "VGGV", "VSSV", "S..."] },
  { par: 92, art: ["SHHS", "VHHV", "VGGV", "VSSV", "V..V"] },
  { par: 116, art: ["VHHV", "VHHV", "VGGV", "VSSV", "S..S"] },
];
...
export function parseArt(art) {
  if (!art || art.length === 0 || art[0].length === 0) {
    throw new Error("a board needs at least one cell");
  }
...
// A board id read back from storage arrives as whatever the watch stored, which
// may be the string "3" rather than the number 3.
function toId(value) {
